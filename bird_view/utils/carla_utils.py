...
class CarlaWrapper(object):
    ...
    def spawn_vehicles(self):
        ...
        for i in range(self.n_vehicles):
            ...
            self.resume_vehicle_behavior(vehicle)
    ...
    def spawn_player(self):
        ...
        self.resume_vehicle_behavior(self._player)
    ...
    def pause_vehicle_behavior(self,vehicle_actor):
        # Check if the actor is a vehicle.
        if not isinstance(vehicle_actor, carla.Vehicle):
            print("Error: Provided actor is not a vehicle (carla.Vehicle).")
            return

        # Set constant throttle and brake inputs to bring the vehicle to a stop.
        vehicle_actor.apply_control(carla.VehicleControl(throttle=0.0, brake=1.0))
        print("Vehicle behavior paused.")

    def resume_vehicle_behavior(self,vehicle_actor):
        # Check if the actor is a vehicle.
        if not isinstance(vehicle_actor, carla.Vehicle):
            print("Error: Provided actor is not a vehicle (carla.Vehicle).")
            return

        # Resume the vehicle's behavior by releasing the brake.
        vehicle_actor.apply_control(carla.VehicleControl(throttle=0.0, brake=0.0))
        print("Vehicle behavior resumed.")
    def clean_up(self):
        for vehicle in self._actor_dict['vehicle']:
            # continue
            self.pause_vehicle_behavior(vehicle)
        ...
        if self._player:
            self.pause_vehicle_behavior(self._player)
        self._player = None
